...
% \begin{mydef}
%   An action of a group $G$ on a set $X$ is \textbf{faithful} if for all $g_1, g_2 \in G$,
%   \[
%     (\forall x \in X, g_1 \cdot x = g_2 \cdot x) \implies g_1 = g_2.
%   \]
% \end{mydef}
This means that different group elements induce different permutations of $X$.
\leancodefile[firstline=26,lastline=30,firstnumber=26]{../lean/GroupAction/Defs.lean}{https://github.com/FrankieeW/GroupAction/blob/v1.0.0/lean/GroupAction/Defs.lean}
